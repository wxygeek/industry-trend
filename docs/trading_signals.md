...
回测引擎使用独立的假突破判定规则，仅对试探仓（30%仓位）生效：

- 收盘价**连续 2 周**低于 MA34（`consecutive_below_ma >= 2`）

**含义**：之前站上均线被认为是"接近突破"，但价格连续跌回均线下方，说明多头力量不足，上攻只是假动作。回测中卖出试探仓止损。
...
| 信号类型 | 信号 | 操作建议 |
|----------|------|----------|
| 预警信号 | 接近突破 / 接近崩盘 | 接近突破时回测买入30%试探仓；接近崩盘时提示关注 |
| 确认信号 | 确认突破 / 确认崩盘 | 确认突破时加仓剩余70%（或全仓买入）；确认崩盘时卖出 |
| 反转信号 | 假突破 / 假崩盘 | 假突破（连续2周低于MA34）卖出试探仓；假崩盘调整预期 |
